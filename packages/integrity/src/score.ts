...
interface ScoreInput {
  /** null/undefined means "unknown yet"; mirrors the Python contract where
   * the ratio penalty is skipped if no heartbeat has landed. Treating
   * unknown as 0 (the previous behavior) over-penalized in-progress
   * previews before the first heartbeat arrived. */
  active_time_seconds: number | null | undefined;
  events: EventCount;
...
export function computeIntegrityScore(input: ScoreInput): number {
  ...
  if ((input.events.devtools_opened ?? 0) > 0) {
    score -= 15;
  }
  ...
  if (
    ...
    input.active_time_seconds != null &&
